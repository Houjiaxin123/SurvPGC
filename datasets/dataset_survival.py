...
class SurvivalDatasetFactory:
    ...
    def __init__(self,
        ...
        self.is_survpath_f = is_survpath_f
        self.is_survpgc = is_survpgc
        self.is_survpgc_f = is_survpgc_f
        ...
        elif self.label_col == "survival_months_pfs":
            self.survival_endpoint = "PFS"
            self.censorship_var = "censorship_pfs"
    ...
    def return_splits(self, args, csv_path, fold):
        ...
        test_split = self._get_split_from_df(args, all_splits=all_splits, split_key='test', fold=fold, scaler=scaler)
...
class SurvivalDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        #@TODO what is the difference between tmil_abmil and transmil_wsi
        ...
        elif self.modality in ["porpoise"]:
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            df_small = self.omics_data_dict["rna"][self.omics_data_dict["rna"]["temp_index"] == case_id]
            df_small = df_small.drop(columns="temp_index")
            df_small = df_small.reindex(sorted(df_small.columns), axis=1)
            ...
            return (patch_features, gene_features, label, event_time, c, clinical_data, mask)

        elif self.modality == "survpgc_f":
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            clinic_features = self._load_clinic_embs_from_prompt(self.clinic_dir, slide_ids)
            gene_features = self._load_gene_embs_from_path(self.gene_dir, slide_ids)
            return (patch_features, gene_features, clinic_features, label, event_time, c, clinical_data, mask)

        elif self.modality == 'survpc':
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
        ...
        elif self.modality == "survpc_f":
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            clinic_features = self._load_clinic_embs_from_prompt(self.clinic_dir, slide_ids)
            return (patch_features, clinic_features, label, event_time, c, clinical_data, mask)

        elif self.modality == "mlppc_concat":
            patch_features, mask = self._load_wsi_embs_from_path(self.data_dir, slide_ids)
            clinic_features = self._load_clinic_embs_onehot(slide_ids)
            return (patch_features, clinic_features, label, event_time, c, clinical_data, mask)
    ...
    def _load_clinic_embs_from_prompt(self, clinic_dir, slide_ids):
        """
        Load all the clinical prompt embeddings from a list a slide IDs.
    ...
    def _load_clinic_embs_onehot(self, slide_ids):
        ...
        clinic_onehot = pd.read_csv(file_path)
        clinic_onehot = clinic_onehot.set_index(['PATIENT_ID'])
        clinic_features = []
        for sample in slide_ids:
            clinic_feature = torch.tensor(clinic_onehot.loc[sample[0:12]])
            clinic_features.append(clinic_feature)
        clinic_features = torch.cat(clinic_features, dim=0)
