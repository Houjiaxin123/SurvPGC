...
Implement a self normalizing network to handle single modality data 
...
class SNNSingle(nn.Module):
    ...
    def forward(self, return_feats=False, **kwargs):
        # ---> unpack
        # x = kwargs["x_omic"].float().squeeze()
        x = kwargs["x_clinic"].float().squeeze()

        h_omic = self.fc_omic(x)
        h_omic = torch.mean(h_omic, dim=0).unsqueeze(dim=0)
